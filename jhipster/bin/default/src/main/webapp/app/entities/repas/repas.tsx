  ...
  return (
    <div>
      <h2 id="repas-heading" data-cy="RepasHeading">
        <Translate contentKey="ecomApp.repas.home.title">Repas</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="ecomApp.repas.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/repas/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="ecomApp.repas.home.createLabel">Create new Repas</Translate>
          </Link>
        </div>
      </h2>
        ...
        {repasList && repasList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="ecomApp.repas.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('date')}>
                  <Translate contentKey="ecomApp.repas.date">Date</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('date')} />
                </th>
                <th className="hand" onClick={sort('calories')}>
                  <Translate contentKey="ecomApp.repas.calories">Calories</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('calories')} />
                </th>
                <th>
                  <Translate contentKey="ecomApp.repas.patient">Patient</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {repasList.map((repas, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{repas.date ? <TextFormat type="date" value={repas.date} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{repas.calories}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="ecomApp.repas.home.notFound">No Repas found</Translate>
            </div>
          )
        )}
      </div>
