...
export default class DockerComposeGenerator extends BaseWorkspacesGenerator {
        ...
        return {
            ...
            checkDockerCompose: runAsync(function () {
                ...
                shelljs.exec('docker compose version', { silent: true }, (code, stdout, stderr) => {
                    if (stderr) {
                        this.log.error(chalk.red('Docker Compose 1.6.0 or later is not installed on your computer.\n' +
                ...
                deployment.appsYaml = applications.map(appConfig => {
                    ...
                    if (yamlConfig.environment) {
                        yamlConfig.environment = yamlConfig.environment.map(envOption => {
                            ...
                            ['JHIPSTER_CACHE_MEMCACHED_SERVERS', 'SPRING_COUCHBASE_CONNECTION_STRING', 'SPRING_CASSANDRA_CONTACTPOINTS'].forEach(varName => {
                                if (envOption.startsWith(varName)) {
    ...
    get end() {
        return {
                ...
                const uiApplications = applications.filter(app => (app.applicationTypeGateway || app.applicationTypeMonolith) && app.clientFrameworkAny);
