...
/**
 * Performance test for the Ehpad entity.
 */
public class EhpadGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all ehpads").get("/api/ehpads").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ehpad")
                        .post("/api/ehpads")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nom\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ehpad_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ehpad").get("${new_ehpad_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created ehpad").delete("${new_ehpad_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Ehpad entity").exec(scn);
