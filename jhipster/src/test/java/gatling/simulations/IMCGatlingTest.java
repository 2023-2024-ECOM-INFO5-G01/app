...
/**
 * Performance test for the IMC entity.
 */
public class IMCGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all iMCS").get("/api/imcs").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new iMC")
                        .post("/api/imcs")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"imc\": 0" + ", \"date\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_iMC_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created iMC").get("${new_iMC_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created iMC").delete("${new_iMC_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the IMC entity").exec(scn);
