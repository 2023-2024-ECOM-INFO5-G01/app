...
/**
 * Performance test for the Note entity.
 */
public class NoteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all notes").get("/api/notes").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created note").get("${new_note_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created note").delete("${new_note_url}").headers(headers_http_authenticated))
                .pause(10)
        );
