 ...
 */
public class PatientGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all patients").get("/api/patients").headers(headers_http_authenticated).check(status().is(200)))
                    ...
                    http("Create new patient")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"taille\": 0" +
                                ", \"datearrive\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_patient_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created patient").get("${new_patient_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created patient").delete("${new_patient_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Patient entity").exec(scn);
