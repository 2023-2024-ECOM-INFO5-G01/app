...
/**
 * Performance test for the Poids entity.
 */
public class PoidsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all poids").get("/api/poids").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new poids")
                        .post("/api/poids")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"poids\": 0" + ", \"date\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_poids_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created poids").get("${new_poids_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created poids").delete("${new_poids_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Poids entity").exec(scn);
