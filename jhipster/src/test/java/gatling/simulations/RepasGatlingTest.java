...
/**
 * Performance test for the Repas entity.
 */
public class RepasGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all repas").get("/api/repas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new repas")
                        .post("/api/repas")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"date\": \"2020-01-01T00:00:00.000Z\"" + ", \"calories\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_repas_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created repas").get("${new_repas_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created repas").delete("${new_repas_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Repas entity").exec(scn);
