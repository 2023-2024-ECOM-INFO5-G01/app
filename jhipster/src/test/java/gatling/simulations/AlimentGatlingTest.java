...
/**
 * Performance test for the Aliment entity.
 */
public class AlimentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all aliments").get("/api/aliments").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new aliment")
                        .post("/api/aliments")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nom\": \"SAMPLE_TEXT\"" + ", \"calories\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_aliment_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created aliment").get("${new_aliment_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created aliment").delete("${new_aliment_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Aliment entity").exec(scn);
