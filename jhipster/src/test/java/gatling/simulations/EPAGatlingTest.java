...
/**
 * Performance test for the EPA entity.
 */
public class EPAGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all ePAS").get("/api/epas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ePA")
                        .post("/api/epas")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"epa\": 0" + ", \"date\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ePA_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ePA").get("${new_ePA_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created ePA").delete("${new_ePA_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EPA entity").exec(scn);
