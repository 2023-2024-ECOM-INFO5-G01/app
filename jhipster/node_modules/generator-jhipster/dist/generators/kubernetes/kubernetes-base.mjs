...
export const checkKubernetes = runAsync(function () {
    ...
    shelljs.exec('kubectl version', { silent: true }, (code, stdout, stderr) => {
        if (stderr) {
            this.log.warn('kubectl 1.2 or later is not installed on your computer.\n' +
...
export const checkHelm = runAsync(function () {
    if (this.skipChecks)
        return;
    const done = this.async();
    shelljs.exec('helm version --client | grep -E "(v2\\.1[2-9]{1,2}\\.[0-9]{1,3})|(v3\\.[0-9]{1,2}\\.[0-9]{1,3})"', { silent: true }, (code, stdout, stderr) => {
        if (stderr || code !== 0) {
